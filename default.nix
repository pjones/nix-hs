...
let
  ...
  pkgs_ = if enableFullyStaticExecutables then pkgs.pkgsMusl else pkgs;
  ...
  lib = import ./nix/lib.nix {
    pkgs = pkgs_ // {
      ...
      haskell = pkgs_.haskell // {
        packages = pkgs_.haskell.packages // { "${compilerName}" = haskell; };
      };
    };
  };
  ...
  compilerName = if compiler == "default" then
    builtins.replaceStrings [ "." "-" ] [ "" "" ] pkgs_.haskellPackages.ghc.name
  else
    "ghc${compiler}";
  ...
  haskell = packageSet.override (orig: {
    overrides = pkgs_.lib.composeExtensions (orig.overrides or (_: _: { }))
      (overrides hlib);
  });
  ...
  drvSansAdditions = cabalFile: haskell:
    lib.addDataFiles haskell addDataFiles (lib.addPostPatch postPatch
  ...
  multiPackage = fileSet:
    ...
    let # fileSet is a attr set where the keys are names of packages and
      ...
      packages = haskell:
        mapAttrs (_: val: drvSansAdditions val haskell) fileSet;
    ...
    in packages haskellExtra // { inherit interactive; };

in if builtins.isAttrs cabal then multiPackage cabal else singlePackage cabal
